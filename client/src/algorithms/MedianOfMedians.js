...
function kthSmallestElement(arr, low, high, k) {
    if (k > 0 && k <= high - low + 1) {
        ...
        if (n % 5 !== 0) {
            median[i] = getMedian(arr.slice(5 * i + low, 5 * i + low + (n % 5)), n % 5);
            i++;
        }

        let medOfMed = (i === 1) ? median[i - 1] : kthSmallestElement(median, 0, i - 1, Math.floor(i / 2));
        ...
        return kthSmallestElement(arr, partition + 1, high, k - (partition + 1) + low);
    }
